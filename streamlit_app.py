...
# --- CONFIGURATION ---
st.set_page_config(page_title="WISC-V Standard (Réunion)", page_icon="🇷🇪", layout="wide")
...
with st.sidebar:
    st.header("📚 Documents")
    st.warning("⚠️ Mode Réunion : Sélectionnez 1 SEUL livre à la fois (sinon erreur mémoire).")
    
    local_files = [f for f in os.listdir('.') if f.lower().endswith(('.pdf', '.txt')) and f not in ["requirements.txt", "app.py"]]
    
    # Par défaut, on ne coche RIEN pour éviter le crash au démarrage
    if local_files:
    ...
    if doc_count > 1:
        st.error("🛑 STOP : Trop de livres sélectionnés. Décochez-en pour n'en garder qu'un seul.")
...
# --- GENERATION ---
if st.button("✨ Analyser (Mode Standard)", type="primary"):
    
    if doc_count > 1:
        st.error("Erreur : Veuillez ne cocher qu'un seul livre dans la colonne de gauche.")
        st.stop()
    ...
    with st.spinner("Analyse en cours..."):
        try:
            # ON UTILISE LE VIEUX MODÈLE SOLIDE
            ...
            TÂCHE:
            Rédiger l'analyse psychométrique (Partie III).
            Si une source théorique est fournie, utilise-la pour justifier.
